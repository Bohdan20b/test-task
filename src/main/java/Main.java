...
public class Main {
    public static void main(String[] args) {
        ...
        // Notice that we can only create user with createUser
        // method without using constructor or builder
        ...
        // Notice that we can only create order with createOrder
        // method without using constructor or builder
        List<Order> orders = new ArrayList<>() {{
                ...
                add(Order.createOrder(user4,
                        List.of(virtualProduct1, virtualProduct2, realProduct1,
                                realProduct2)));
            }
        };
        ...
        CodeManager virtualProductCodeManager = CodeManager.getInstance();
        virtualProductCodeManager.useCode("xxx");
        System.out.println(virtualProductCodeManager.isCodeUsed("xxx"));
        System.out.println(virtualProductCodeManager.isCodeUsed("yyy"));

        //TODO 3). Create a functionality to get the most popular product
        // (product bought by most users) among users
    ...
    private static Product getMostExpensiveProduct(List<Order> orders) {
        return orders.stream()
                ...
                .orElseThrow(() -> new RuntimeException("Can't find product with max price."));
    }

    private static Product getMostPopularProduct(List<Order> orders) {
        Map<Product, Long> productCounts = orders.stream()
                .flatMap(order -> order.getProductList().stream())
        ...
        return productCounts.entrySet().stream()
                ...
                .orElseThrow(() -> new RuntimeException("Can't find most popular product."));
    }
    ...
    private static Map<Order, Integer> calculateWeightOfEachOrder(List<Order> orders) {
        ...
        for (Order order : orders) {
            int totalWeight = order.getProductList().stream()
                    .filter(product -> product instanceof Real)
                    .mapToInt(product -> ((Real) product).getWeight())
                    .sum();
